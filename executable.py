...
def hadoop_installer( version, root_dir ):
  ...
  fd = tarfile.open( 'hadoop-%s.tar.gz' % version, 'r:gz' )
  try:
    fd.extractall()
  finally:
    fd.close()
...
def hive_installer( version, root_dir ):
  ...
  fd = tarfile.open( 'apache-hive-%s-bin.tar.gz' % version, 'r:gz' )
  try:
    fd.extractall()
  finally:
    fd.close()
  ...
  fd = tarfile.open( 'db-derby-10.14.2.0-bin.tar.gz', 'r:gz' )
  try:
    fd.extractall()
  finally:
    fd.close()
  ...
  fd = tarfile.open( 'sqoop-1.4.7.bin__hadoop-2.6.0.tar.gz', 'r:gz' )
  try:
    fd.extractall()
  finally:
    fd.close()
  ...
  fd = tarfile.open( 'mysql-connector-java-8.0.20.tar.gz', 'r:gz' )
  try:
    fd.extractall()
  finally:
    fd.close()
...
<configuration>\n\
  ...
  copyfile( os.path.join( os.environ['HIVE_HOME'], 'lib/hive-common-2.3.7.jar' ),
            os.path.join( os.environ['SQOOP_HOME'], 'lib/hive-common-2.3.7.jar' ) )
...
def pig_installer( version, root_dir ):
  ...
  fd = tarfile.open( 'pig-%s.tar.gz' % version, 'r:gz' )
  try:
    fd.extractall()
  finally:
    fd.close()
...
def spark_installer( version, root_dir, hadoop_version, hive_version):
  # ~ spark_url = 'https://archive.apache.org/dist/spark/spark-%s/spark-%s-bin-without-hadoop.tgz' % (version, version)
  spark_url = 'https://archive.apache.org/dist/spark/spark-%s/spark-%s-bin-hadoop%s.tgz' % (version, version, hadoop_version)

  urllib.request.urlretrieve( spark_url, 'spark-%s-bin-hadoop%s.tgz' % (version, hadoop_version) )

  fd = tarfile.open( 'spark-%s-bin-hadoop%s.tgz' % (version, hadoop_version), 'r:gz' )
  try:
    fd.extractall()
  finally:
    fd.close()

  subprocess.run( ['ln', '-s', 'spark-%s-bin-hadoop%s' % (version, hadoop_version), 'spark' ] )

  subprocess.run( ['pip', 'install', '-q', 'findspark'] )

  if 'PATH' not in os.environ:
    os.environ['PATH'] = ''

  ## Hive support
  #spark_hive_url = 'https://repo1.maven.org/maven2/org/apache/spark/spark-hive_2.11/%s/spark-hive_2.11-%s.jar' % (version, version)
  #urllib.request.urlretrieve( spark_url, 'spark-hive_2.11-%s.jar' % version )

  os.environ['SPARK_HOME'] = os.path.join( root_dir, 'spark' )
  os.environ['PATH'] = os.environ['PATH'] + ':/content/spark/bin'

  process = subprocess.Popen( ['hadoop', 'classpath'], stdout = subprocess.PIPE )
  (output, err) = process.communicate()
  exit_code = process.wait()
  os.environ['SPARK_DIST_CLASSPATH'] = output.decode( 'utf-8' )
  
  if os.path.exists('apache-hive-%s-bin/conf/hive-site.xml' % hive_version):  
    copyfile('apache-hive-%s-bin/conf/hive-site.xml' % hive_version,'spark-%s-bin-hadoop%s/conf/hive-site.xml' % (version, hadoop_version))
    copyfile('apache-hive-%s-bin/conf/hive-site.xml' % hive_version,'spark/conf/hive-site.xml')
...
if __name__ == '__main__':
  ...
  hadoop_installer( '2.7.0', '/content' )
  hive_installer( '2.3.7', '/content' )
  #pig_installer( '0.17.0', '/content' )
  spark_installer( '2.4.6', '/content' , hadoop_version='2.7', hive_version='2.3.7')